...
namespace BilliardCruise.Sava.Scripts
{
    ...
    public class PoolManager : MonoBehaviour
    {
        ...
        protected void RackBalls()
        {
            ...
            List<Ball> shuffledBalls = PoolUtils.Shuffle(Balls);
        ...
        public bool IsTouchZone1(Vector3 touchPosition)
        {
            if (cueBallIndicatorRenderer == null)
            {
                return false;
            ...
            return (Vector3.Distance(CueBall.transform.position, touchPosition) >= cueBallIndicatorRenderer.bounds.size.x / 2f) && (Vector3.Distance(CueBall.transform.position, touchPosition) <= cueBallIndicatorRenderer.bounds.size.x * 2f);
        }

        public bool IsTouchZone2(Vector3 touchPosition)
        {
            if (cueBallIndicatorRenderer == null)
            {
                return false;
            }
            return (Vector3.Distance(CueBall.transform.position, touchPosition) >= cueBallIndicatorRenderer.bounds.size.x * 2f) && (Vector3.Distance(CueBall.transform.position, touchPosition) <= cueBallIndicatorRenderer.bounds.size.x * 3f);
        }

        public bool IsTouchZone2Out(Vector3 touchPosition)
        {
            if (cueBallIndicatorRenderer == null)
            {
                return false;
            }
            return (Vector3.Distance(CueBall.transform.position, touchPosition) >= cueBallIndicatorRenderer.bounds.size.x * 3f);
        }
        public float GetHitBallStrength(Vector3 startPosition, Vector3 touchPosition)
        {
            ...
            if (Vector3.Distance(CueBall.transform.position, startPosition) < Vector3.Distance(CueBall.transform.position, touchPosition))
            {
                return 0f;
            }


            return (1f - (Vector3.Distance(touchPosition, CueBall.transform.position)) / (Vector3.Distance(startPosition, CueBall.transform.position)));
        }
